...
class phpthumb {
	...
	var $config_imagemagick_path                     = '/usr/bin/convert';
	...
	var $config_allow_src_above_docroot              = true; //for shops.car1.hk
					...
					switch ($command) {
						...
						case 'usm':
							if ($this->ImageMagickSwitchAvailable('unsharp')) {
								...
								$commandline .= ' -unsharp '.escapeshellarg( number_format($radius * 3  , 2, '.', ''). 'x' .number_format(($threshold / 4), 3, '.', '').'+'.number_format($amount / 25, 2, '.', '') . '+0') ;
								$successfullyProcessedFilters[] = $filterkey;
							}
							break;
