...
class phMagick_resize{
    function resize(phmagick $p , $width, $height = 0, $exactDimentions = false, $rotate = false){
		...
		//GraphicsMagicK cannot use this operation -strip
        $cmd .=  ' -strip ';
    ...
    function resizeExactly(phmagick $p , $width, $height){
		...
		//var_dump ( $p->getInfo($p->getSource()) );

		$temp_size = $p->getInfo($p->getSource());

		list($w, $h, $b) = $temp_size;
		$w = (int) $w;
		$h = (int) $h;
		$b = (int) $b;

		//file_put_contents( '/var/www/html/resize-log.txt', var_dump($temp_size) ."\n" , FILE_APPEND );

    	//list($w,$h) = $p->getInfo($p->getSource());

		//file_put_contents( '/var/www/html/resize-log.txt', $w.'x'.$h ."\n" , FILE_APPEND );
